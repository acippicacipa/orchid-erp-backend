...
class DataImportService:
    ...
    def _validate_product_row(self, row_number, row):
        ...
        try:
            ...
            if price < 0:
                self._add_error(row_number, 'price', 'VALIDATION_ERROR', 'Price cannot be negative')
                is_valid = False
        except (ValueError, TypeError):
            self._add_error(row_number, 'price', 'DATA_TYPE_ERROR', 'Invalid price format')
            is_valid = False
        
        # Validate category if provided
        category_name = row.get('category')
        if category_name and not pd.isna(category_name):
            if not Category.objects.filter(name=str(category_name).strip()).exists():
                self._add_error(row_number, 'category', 'FOREIGN_KEY_ERROR', f'Category "{category_name}" does not exist')
                is_valid = False
    ...
    def _import_product_row(self, row_number, row):
        ...
        product = Product.objects.create(
            ...
            price=float(row.get('price', 0)),
            cost=float(row.get('cost', 0)) if row.get('cost') and not pd.isna(row.get('cost')) else None,
            category=category,
    ...
    def create_default_templates():
        ...
        templates = [
            ...
            },
            {
                ...
                'optional_columns': ['description', 'cost', 'category', 'unit'],
                'column_mappings': {
                    'name': 'name',
                    'sku': 'sku',
